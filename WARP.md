...
- `application/main.tf` - Creates:
  - Backend namespace (`2fa-app`) for application pods
  - LDAP admin secret in backend namespace (for LDAP authentication)
  - PostgreSQL secret copy in backend namespace (for database access)
  - Redis secret copy in backend namespace (for SMS OTP storage, if enabled)
  ...
  - ArgoCD Applications (if enabled, depends on all secrets being created first)
...
- `TF_VAR_OPENLDAP_ADMIN_PASSWORD` - OpenLDAP admin password (used for OpenLDAP
deployment and backend namespace `ldap-admin-secret`)
- `TF_VAR_OPENLDAP_CONFIG_PASSWORD` - OpenLDAP config password (for OpenLDAP
deployment)
...
- **Destroy Workflow Confirmation**:
  - All destroy workflows now require explicit confirmation input
  - When running a destroy workflow in GitHub Actions, you must type `yes` in the
  "Type 'yes' to confirm destruction" input field
  - Workflow fails with clear error message if confirmation is not provided
  - Reduces risk of accidental infrastructure destruction
  - Affected workflows: Application, Application Infrastructure, Backend Infrastructure,
  TF Backend State
...
- **Backend Namespace Secrets**:
  ...
  - Secrets are populated from:
    ...
    - AWS Secrets Manager (`tf-vars` secret) for local script execution

- **Build Workflow Image Tag Improvements**:
  - Backend and frontend build workflows now generate unique image tags using both
  commit SHA and GitHub run ID: `<image-name>-<sha>-<run_id>`
  - Prevents ECR tag conflicts when re-running workflows (ECR tags are immutable)
  - Each workflow run produces a distinct tag that can be pushed to ECR
  - Helm values are automatically updated and committed with new image tag
  - Git push includes retry logic with up to 3 attempts for concurrent workflow
  handling
...
- **ArgoCD Module Access Entry and Cluster Role**:
  ...
  - Added `aws_eks_access_policy_association.argocd_capability_cluster_admin` to
  grant ArgoCD Capability IAM role full cluster admin via `AmazonEKSClusterAdminPolicy`
  - Access entry is created automatically by EKS when capability is created; module
  only associates the access policy with the principal ARN
  - Ensures ArgoCD can sync applications and manage resources across all namespaces

- **Backend Dockerfile Corrections**:
  - Corrected Python LDAP library usage (libldap2-dev, libsasl2-dev for build;
  libldap-2.5-0, libsasl2-2 for runtime)
...
- **ArgoCD Module Enhancements**:
  ...
  - Added single wait (`time_sleep.wait_for_argocd`, default 330s) for ArgoCD
