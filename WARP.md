...
- **EKS Auto Mode**: Simplified cluster management with automatic node provisioning and built-in EBS CSI driver
- **Networking**: Single NAT gateway (cost optimization), IGW, VPC endpoints for SSM access
- **Storage**: EBS-backed persistent volumes with `gp3` type and encryption enabled (created in backend_infra)
- **Container Registry**: ECR repository for Docker images with lifecycle policies
- **DNS & Certificates**: Uses existing Route53 hosted zone and validated ACM certificate
...
- `backend_infra/variables.tfvars` - Configure VPC CIDR, K8s version, resource names, ECR lifecycle policies
...
- `backend_infra/modules/` - Reusable modules for EBS, ECR, and VPC endpoints

### Application Layer
- `application/variables.tfvars` - Configure domain name, OpenLDAP PVC name (passwords via .env file)
- `application/helm/openldap-values.tpl.yaml` - Helm chart values template with Ingress annotations
- `application/providers.tf` - Retrieves cluster name from backend_infra remote state
- `application/main.tf` - Creates Helm release, Route53 records, and network policies
...
- Ingress annotations control ALB behavior (scheme: internet-facing, target-type: ip, certificates)
- Both Ingress resources use the same ALB with host-based routing
- Hostnames automatically derived: `phpldapadmin.${domain_name}` and `passwd.${domain_name}`

### Resource Dependencies
Deployment order matters:
1. Terraform backend state infrastructure (S3 bucket)
2. Backend infrastructure (VPC → EKS → VPC Endpoints → EBS StorageClass/PVC → ECR)
3. Application layer (Existing Route53 zone + ACM cert lookup → Helm release → Route53 A records → Network policies)

### Module Structure
- `backend_infra/modules/` - Reusable infrastructure modules (ebs, ecr, endpoints)
- `application/modules/` - Application-specific modules (alb, network-policies)
- Each module has its own README.md with detailed documentation
- Route53 and ACM certificate resources now use data sources instead of dedicated module
...
- `OPENLDAP_ADMIN_PASSWORD` - OpenLDAP admin password (for application workflows)
- `OPENLDAP_CONFIG_PASSWORD` - OpenLDAP config password (for application workflows)
...
- **Cluster name not found**: Ensure backend_infra is deployed first and `backend.hcl` is configured correctly
...
- **ALB not created**: Check Ingress resources have proper annotations and ACM certificate is validated
- **Route53 DNS not resolving**: Ensure Route53 A records point to ALB DNS name and NS records are configured at registrar

## Security Best Practices (from .cursor/rules)
